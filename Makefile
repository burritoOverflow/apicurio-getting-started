...
CREATE_ARTIFACT_BIN= $(BIN_DIR)/create-artifact

CREATE_ITEM_SRC = ./client/create-item.go
CREATE_ITEM_BIN = $(BIN_DIR)/create-item
...
build: $(CREATE_ARTIFACT_BIN) $(SEARCH_ARTIFACT_BIN) $(CREATE_ITEM_BIN)
...
$(CREATE_ITEM_BIN): $(CREATE_ITEM_SRC)
	@echo "Building create-item..."
	@mkdir -p $(BIN_DIR)
	$(GO) build -o $@ $(CREATE_ITEM_SRC)
